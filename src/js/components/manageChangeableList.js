...
export const initChangeableLists = () => {
  const changeablePage = document
    .querySelector('ul[data-list="changeable"]')
    ?.closest('main');
  ...
  if (changeablePage) {
    ...
    changeablePage.addEventListener('click', (e) => {
      if (e.target.dataset.btn === 'delete') {
        ...
        const changeableLists = changeablePage.querySelectorAll(
      ...
      if (e.target.dataset.btn === 'add') {
        const targetChangeableList = e.target.parentElement.querySelector(
          'ul[data-list="changeable"]',
        );
        // let changeableInputs = targetChangeableList.querySelectorAll(
        ...
        //   showInfoModal(
        //     'Для создания нового элемента необходимо заполнить все предыдущие поля!',
        //   );
        //   return;
        // }
        ...
        const changeableElements =
          targetChangeableList.querySelectorAll('li.changeable-item');
        initInputs();
